...
export default function SceneBuilder({ scenes }) {
  ...
  useEffect(() => {
    // If no scenes are provided, don't run GSAP to avoid "target null" errors
    if (!scenes || scenes.length === 0) return;

    const ctx = gsap.context(() => {
      // 1. Reveal text and lines
    ...
    return () => ctx.revert();
  }, [scenes]); // Re-run if the scenes array changes
  ...
  return (
    <section ref={containerRef} className="max-w-[1440px] mx-auto px-6 space-y-40 pb-40 mt-40">
      {scenes.map((step, i) => (
        <div 
          key={i} 
          className={`reveal-section flex flex-col ${i % 2 !== 0 ? 'md:items-end' : 'md:items-start'}`}
        >
          
          {/* Image Box */}
          <div className={`relative overflow-hidden bg-primary/5 shadow-2xl ${
            step.type === 'wide' ? 'w-full aspect-video' : 'w-full md:w-2/3 aspect-[3/4]'
          }`}>
          ...
          {/* Text Box */}
          <div className={`mt-12 max-w-xl space-y-4 ${i % 2 !== 0 ? 'md:text-right' : ''}`}>
            ...
            <div className={`reveal-item h-px w-24 bg-primary/20 ${i % 2 !== 0 ? 'ml-auto' : ''}`} />
          </div>
